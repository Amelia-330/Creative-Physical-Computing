...
int remindTime = 1000;//void loop中的程序运行100次所花的时间
...
void loop() {
  ...
  if ((analogRead(forcePin) >= forceValue) && (1.5 * remindTime > timer >= remindTime)) {
  ...
  else if ((analogRead(forcePin) >= forceValue) && (2 * remindTime > timer >= 1.5 * remindTime)) {
  ...
  else if ((analogRead(forcePin) >= forceValue) && (timer >= 2 * remindTime)) {
