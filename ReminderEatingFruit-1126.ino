...
int remindTime = 30000;//void loop中的程序运行2次所花的时间
...
int forceValue = 4;// range from 0-1023 // 等苹果泡沫模型到了用Serial序列部测试一下，多大值合适
...
void loop() {
  ...
  else if ((analogRead(forcePin) >= forceValue) && (4 * remindTime > timer >= 2 * remindTime)) {
    myservo1.write(4 * val); // if force sensor is HIGH, after 10s-30s， then servo 20 degree
    myservo2.write(4 * val); // if force sensor is HIGH, after 10s-30s， then servo 20 degree
    timer++;
  }
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////触摸传感器启动30秒后，伺服器带动盘子倾斜最大角度
  else if ((analogRead(forcePin) >= forceValue) && (timer >= 4 * remindTime)) {
